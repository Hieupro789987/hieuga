...
export function ExpertForgotPasswordPage() {
  ...
  const handleSubmit = async ({ email }) => {
    ...
    try {
      ...
      const res = await ExpertService.resetExpertPasswordByEmailSendLink(email);
      if (res) {
        setIsSentEmail(true);
      }
    } catch (error) {
  ...
  return (
    <AuthenticateLayout
      ...
      title={title}
      subTitle={subtitle}
      {...(isSentEmail && { className: "px-5 pt-2" })}
    >
      <Form className="flex flex-col p-6 rounded max-w-screen-xs m-auto" onSubmit={handleSubmit}>
