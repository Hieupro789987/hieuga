...
export function ExportCollaboratorsDialog({ memberId, ...props }: PropsType) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await saveFile(
        () =>
          CollaboratorService.exportExcelAdmin(
            formatDate(fromDate, "yyyy/MM/dd"),
            formatDate(toDate, "yyyy/MM/dd"),
            memberId
          ),
        "excel",
        `CONG_TAC_VIEN_TU_${formatDate(fromDate, "dd-MM-yyyy")}_DEN_${formatDate(
          toDate,
          "dd-MM-yyyy"
        )}${
          memberFullData
  ...
  return (
    <Form
      dialog
      title="Xuất danh sách cộng tác viên"
      isOpen={props.isOpen}
