...
export function WriterResetPasswordPage() {
  ...
  const checkUrl = useMemo(async () => {
    ...
    try {
      const res = await WriterService.resetWriterPasswordByEmailCheckLink(code as string);
      if (res) return true;
      return false;
    } catch (error) {
      console.log(error);
  ...
  const handleSubmit = async ({ password }) => {
    ...
    try {
      const res = await WriterService.resetWriterPasswordByEmail(code as string, password);
      if (res) {
...
function ResetPasswordPart() {
  return (
    <>
      <Field className="mb-1 group" name="password" required>
        <Input
          className="h-12 w-96"
          type="password"
